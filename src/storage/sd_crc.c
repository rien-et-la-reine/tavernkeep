...
uint8_t crc_helper_7(const uint8_t *message, size_t length) {
    ...
    uint8_t mask = 0x12;
    //create eval register, read in first message byte
    uint8_t eval = 0;
    ...
    //7 bits left -> CRC value (return raw CRC value, processing for transmission should happen in context, not here)
    return eval >> 1;
}
